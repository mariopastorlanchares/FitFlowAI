...
- `2026-04-02`: Hardening runtime adicional tras prueba manual en Android: el cliente espera la hidratación inicial real de Firebase Auth antes de liberar navegación, y la validación de `GeneratedWorkoutSession` normaliza opcionales `null` del transporte callable (`summary`, `session_notes`, `title`, `coach_notes`) a `undefined` para no degradar a preview por un shape válido semánticamente pero serializado distinto. Verificado con `npx tsc --noEmit`, `npx expo lint src/features/auth src/shared/lib __tests__/auth-service.test.ts __tests__/generator-contract.test.ts __tests__/workout-service.test.ts` y `npx jest __tests__/auth-service.test.ts __tests__/auth-screens.test.tsx __tests__/generator-contract.test.ts __tests__/workout-service.test.ts --runInBand`.
