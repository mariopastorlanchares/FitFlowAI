...
- [x] **Acción:** Preparar criterios de prueba para asegurar que las restricciones se respetan
- [x] **Archivos afectados:** `__tests__/exercise-compatibility.test.ts`, `__tests__/generator-contract.test.ts`, `__tests__/workout-history.test.ts`, `__tests__/firestore.rules.test.ts`
- [x] **Detalles:** La validación de compatibilidad ya cubre dominio y generador, este slice añade tests puros del snapshot de historial y reglas para `workoutSessions`, y la batería de reglas ya se ha ejecutado materialmente en emulador Firestore. La cobertura futura de cambios de contexto/historial más avanzados ya no bloquea este paso V1.
...
- `2026-04-02`: Validación material de reglas cerrada: `__tests__/firestore.rules.test.ts` pasa ya en emulador Firestore con las reglas actuales de `userProfiles` y `workoutSessions`. Para esta máquina fue necesario usar un JDK 21 local temporal porque `firebase-tools` ya no acepta Java 17 para emuladores.
